...
* it's equal to -1 if the i-th item belongs to no group. The items and the groups are zero indexed (Group can have no item)
* Return a sorted list of the items such that:
* Items that belong to the same group are next to each other in the sorted list.
* beforeItems[i] is list containing all items that should come before i-th item in the sorted array (to left of i-th item)
