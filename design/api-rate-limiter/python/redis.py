...
class SlidingWindowCounterRateLimiter(object):
  """
  representation of data stored in redis metadata
  --------
  "userid_metadata": {
    "requests": 2,
    "window_time": 30
  ...
  # Adds a new user's rate of requests to be allowed. using redis hashes to store the user metadata.
  def addUser(self, userId, requests=100, windowTimeInSec=60):
    ...
    self.con.hmset(userId + self.METADATA_SUFFIX, {self.REQUESTS: requests, self.WINDOW_TIME: windowTimeInSec})
  ...
  # Atomically increments hash key val by unit and returns. Uses optimistic locking over userId + self.COUNTS redis key.
  def _atomic_incrementAHashKeyValByUnit(self, userId, bucket, redisPipeline):
  ...
  # Deciding if the rate has been crossed. we're using redis hashes to store the counts.
  def shouldAllowServiceCall(self, userId):
    ...
    currentTimestamp = self.current_time_in_sec()
    ...
    # Changes in _atomic_incrementAHashKeyValByUnit are committed only if none of these entries get changed.
    _, requests = self.con.transaction(
        lambda pipe: self._atomic_incrementAHashKeyValByUnit(userId, currentBucket, pipe), userId + self.COUNTS,
        userId + self.METADATA_SUFFIX)
