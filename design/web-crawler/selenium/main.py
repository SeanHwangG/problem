...
def get_source(problemList, option, userId, cookies):
  ...
  for problem in problemList:
    response = requests.get(f"https://www.acmicpc.net/status?from_mine=1&problem_id={problem}&user_id={userId}",
                            cookies=cookies).text
    soup = BeautifulSoup(response, 'html.parser')
    trs = soup.select(
        'html > body > div.wrapper > div.container.content > div.row > div.col-md-12 > div.table-responsive > table#status-table > tbody > tr'
    )
    tds = trs[0].select('td')
    ...
    if not os.path.exists('./{}/{}~{}'.format(dirname,
                                              int(problem) // 1000 * 1000, (int(problem) // 1000 + 1) * 1000 - 1)):
